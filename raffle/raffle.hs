...
main = do
  ...
  echo s_address
  echo s_collateral
  let runtimeURI = RuntimeURI "174.29.45.142" 3701 -- "0.0.0.0" 56063
      raffleConfiguration =
        RaffleConfiguration
      ...
      deadlines = Deadlines "10m" "10m" "10m"
  ...
  where
    generateContract = do
      marlowe_cli
        ...
        (o_address oracle)
        "--chunk-size"
        (chunkSize raffleConfiguration)
        (asArg $ (\party -> ["--parties", party]) <$> parties)
        "--deposit-deadline"
        (deposit deadlines)
        "--select-deadline"
        (selectWinner deadlines)
        "--payout-deadline"
        (payout deadlines)
    ...
    initialize raffleLoadedHash = do
      marlowe_runtime_cli
        ...
        (s_address sponsor)
        "--collateral-utxo"
        (s_collateral sponsor)
        "--manual-sign"
        (tmpTxToSign raffleConfiguration)
        "--contract-hash"
        raffleLoadedHash
